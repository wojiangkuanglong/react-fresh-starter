...
import { createGetUserByNameQueryResponse } from '../../faker/user/createGetUserByName.ts';
import type {
  GetUserByName400,
  GetUserByName404,
...
export function getUserByNameHandler(
  ...
  return http.get('/user/:username', function handler(info) {
    ...
    return new Response(JSON.stringify(data || createGetUserByNameQueryResponse(data)), {
      status: 200,
      headers: {
        'Content-Type': 'application/json',
      },
